...
class CommandRGBStream(CommandType):
    def __init__(self):
        ...
        self.starting_led:int = 0 # 8 bytes
        self.transition_duration:float = 0 # 8 bytes double in cpp
    ...
    def to_bytes(self):
        ...
        data += self.starting_led.to_bytes(8, byteorder="big")
        data += struct.pack(">d", self.transition_duration)
...
class Discovery: # this is only relevent if you are decoding the message
    ...
    def to_bytes(self):
        data = b""
        data += self.led_number.to_bytes(8, byteorder="big")
        return data
