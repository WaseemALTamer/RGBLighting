...
namespace LightingManager{
    ...
    uint32_t pack_rgb(float r, float g, float b) {
    ...
    void pack_all_leds(){
        // this function perpose is to recalculate all the
        // leds value affter you apply leds manually
        for (int i = 0; i < LEDS_NUMBER; i++) {
            LEDController::LED& led = leds[i];
            CRGB& fast_led = fast_leds[i];
            fast_led = pack_rgb(led.r, led.g, led.b);
        }
    ...
    static void on_led_callback(float r, float g, float b, void* ctx){
        CRGB* self = (CRGB*)ctx;
        if (!self) return;
        *self = pack_rgb(r, g, b); // apply the value now
